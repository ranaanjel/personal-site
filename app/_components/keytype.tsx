...
export default function KeyTypeAnimation({text}: {text?: string}) {
    ...
    useGSAP(() => {
        ...
        gsap.timeline({ repeat:-1, repeatDelay:0.5}).to(textRef.current, {
                text: {
                    value : words[0], type: "diff"
                }, 
                duration: 1.5, ease:"power2.inOut"
            }).to(textRef.current, {
                text: {
                    value : "", type: "diff", rtl: true
                }, 
                duration: 0.5, ease:"power1.out"
            }).to(textRef.current, {
                text: {
                    value : words[1], type: "diff"
                },
                duration: 1.5, ease:"power2.inOut"
            }).to(textRef.current, {
                text: {
    ...
    return <div className="relative inline">
        ...
        <span ref={cursorRef} className="font-extrabold text-xl ml-[0.5ch]">|</span>
    </div>
}
...
export  function AnimateTextLightBulb({text}:{text:string}) {
    ...
    useGSAP(() => {
        gsap.to( textRef.current, {
             ease: "power1.out", duration: 3, text : {
                value: text, type: "diff", delimiter : " "
            }
        })
    })
    return <div className="relative inline mt-2">
        <span ref={textRef} className="text-lg text-gray-800 dark:text-zinc-300/70"></span>
